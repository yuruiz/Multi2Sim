...
#define EVICTION_THRESHOLD_CYCLES 1000
        ...
        if (ctx->confidence > 0 && !self->next_ctx)
        { 
			...
			if (pred_distance > 0 && (pred_distance < EVICTION_THRESHOLD_CYCLES))
			{
				if (ctx->confidence > EVICTION_THRESHOLD_MIN_CONF) 
				{
					...
					DOUBLE_LINKED_LIST_FOR_EACH(self->cpu->emu, context, tmp_ctx)
					{
						/* No affinity */
	...
	if (!self->ctx)
	{
        ...
        if (best_ctx)
		{
			fprintf(stderr,"Yurui: bext ctx selected #%lld ctx %d allocated to thread %s\n",
